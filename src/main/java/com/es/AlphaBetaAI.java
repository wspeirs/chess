...
public class AlphaBetaAI {
    ...
    public int alphabeta(MoveNode node, int depth, int alpha, int beta, Color color) {
        ...
        if(colorPlaying.equals(color)) {
            ...
            for(int p:pieces) {
                ...
                for(int m:moves) {
                    ...
                    try {
                        ...
                        beta = Math.max(alpha, alphabeta(childNode, depth - 1, alpha, beta, color.equals(Color.WHITE) ? Color.BLACK : Color.WHITE));

                        // by here we've recursed down
                        node.addChild(childNode);  // add the new node

                        if(beta <= alpha) {
                            LOG.debug("{} <= {}; RETURNING BETA", beta, alpha);
            ...
            LOG.debug("NORMAL BETA RET: {}", beta);
            return beta;
        }
    }
